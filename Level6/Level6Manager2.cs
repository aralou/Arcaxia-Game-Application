...
public class Level6Manager2 : MonoBehaviour
{
    [Header("UI Elements")]
    ...
    private void Start()
    {
        ...
        if (SceneManager.GetActiveScene().name == "Level2-5")
        {
            ShowCertificate();
        }
        else
        {
            Debug.LogWarning($"Current Scene is NOT Level2-5: {SceneManager.GetActiveScene().name}");
        }
    }
    ...
    private void DisplayPerformance(int score)
{
    ...
    {
        PlayerPrefs.SetInt("Level3Unlocked", 1); // Unlock Level 3
        PlayerPrefs.Save();
    ...
    {
        // Reset level scores and lock Level 3
        ...
        PlayerPrefs.SetInt("Level3Unlocked", 0); // Lock Level 3
        PlayerPrefs.Save();

        Debug.Log("Game state reset. Level 3 locked.");
    }
}
