...
public class Level6Manager : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        if (SceneManager.GetActiveScene().name == "Level1-6")
        {
            ShowCertificate();
        }
        else
        {
            Debug.LogWarning($"Current Scene is NOT Level6: {SceneManager.GetActiveScene().name}");
        }
    }
    ...
    private void DisplayPerformance(int score)
    {
        ...
        // Check and unlock Level 2 based on performance
        LevelManager.Instance?.CheckAndUnlockLevel2();
    ...
    {
        // Reset level scores and lock Level 2
        ...
        PlayerPrefs.SetInt("Level2Unlocked", 0); // Lock Level 2
        PlayerPrefs.Save();

        Debug.Log("Game state reset. Level 2 locked.");
    }
}
