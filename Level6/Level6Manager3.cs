...
public class Level6Manager3 : MonoBehaviour
{
    [Header("UI Elements")]
    ...
    private void Start()
    {
        ...
        if (SceneManager.GetActiveScene().name == "Level3-5")
        {
            ShowCertificate();
        }
        else
        {
            Debug.LogWarning($"Current Scene is NOT Level3-5: {SceneManager.GetActiveScene().name}");
        }
    }
    ...
    private void ResetGameState()
    {
        ...
        PlayerPrefs.SetInt("Level4Unlocked", 0); // Lock Level 3
        PlayerPrefs.Save();

        Debug.Log("Game state reset. Level 4 locked.");
    }
}
